  ...
  async ({
    ...
    try {
      const resp = axiosInterceptorWithCybertoken.get(
        `/api/users/phan-trang-tim-kiem?pageIndex=${pageIndex}&pageSize=8${
...
export const adminUserSlice = createSlice({
  ...
  reducers: {
    updateUserList: (state, action) => {
      const changeUserIndex = state.currentUserbyPhanTrang.data.findIndex(
        (user: IProfile) => user.id === action.payload.id,
      );
      state.currentUserbyPhanTrang.data[changeUserIndex] = action.payload;
    },

    deleteUserFromList: (state, action) => {
      const indexById = state.currentUserbyPhanTrang.data.findIndex(
        (user: IProfile) => user.id === action.payload[0],
      );
      if (indexById != -1) {
  ...
  extraReducers: (build) => {
    build.addCase(getUserByPhanTrang.fulfilled, (state, action) => {
      state.currentUserbyPhanTrang = action.payload?.data;
    });
  },
});
