...
function Register() {
  ...
  const formik = useFormik({
    ...
    validationSchema: Yup.object().shape({
      ...
      password: Yup.string()
        ...
        .matches(
          regex.password,
          "password must contain at least 1 digit, 1 special character, 1 alphabeltic character !",
        ),
    ...
    onSubmit: async (values: IValues) => {
      try {
        ...
        const getGender =
          genderValue === "true" ? true : genderValue === "false" ? false : "";
        ...
        if (date === "undefined-1-undefined" || getGender === "") {
          ...
          const newValue = {
            ...values,
            username: values.user, // Đổi `user` thành `username`
            birthday: date,
            role: "USER",
            gender: getGender,
          };
          await axiosInterceptorWithCybertoken.post(
            "/api/auth/signup",
            newValue,
          );
  ...
  return (
    <div className="register-page">
      {!isValid ? (
        <Alert className="mui-alert" severity="error" sx={{ fontSize: "2rem" }}>
      ...
      <form action="" className="login-form" onSubmit={formik.handleSubmit}>
        ...
        <FormControl
          ...
          className="mui-form-control"
          margin="dense"
          error={formik.errors.checkPassword ? true : false}
        >
          <InputLabel htmlFor="my-input-password-check">
            Nhập lại mật khẩu
          </InputLabel>
          <Input
            id="my-input-password-check"
            aria-describedby="my-helper-text"
