...
export default function SelectVariants({ khachMax, giaTien, phone, dataDetail }: IProps) {
  ...
  const [inputGuest, setInputGuest] = React.useState("1 khách");
      ...
      if (getLocal(ACCESS_USER_ID)) {
        if (availableCount < 1) {
          ...
          try {
            ...
            const res = await axiosInterceptorWithCybertoken.post("/api/dat-phong", value)
            console.log(res.data)
            if (paymentMethod === "pay_online") {
              const ress = await axiosInterceptorWithCybertoken.post("/api/dat-phong/payment-with-momo", {
                total: giaTien * dateDifferent,
                orderId: res.data.newBooking.orderId,
              })
              setDateEnd("")
              setDateStart("")
              setGuest(1)
              window.location.href = ress.data.payUrl
              return 
             }
            setDateEnd("")
            setDateStart("")
  ...
  return !phone ? (
    <div className='my-3'>
      ...
      <FormControl variant="filled" sx={{ color: "black", width: "100%", border: "solid 1px", padding: "0.5rem", borderRadius: "0 0 10px 10px" }}>
        <InputLabel id="demo-simple-select-filled-label" sx={{ fontSize: "2rem", color: "black" }}>{guest} Khách</InputLabel>
        <Select
        ...
        >
          ...
          <p className="guest-description">Nơi này có tối đa {khachMax} người</p>
          <button className="guest-close mb-4" style={{ marginLeft: "80%" }}>Đóng</button>
        </Select>
      </FormControl>
      <button className="detail-submit-guest" type='button' onClick={() => handleBooking('pay_at_property')}>
      Thanh toán tại chỗ
      </button>
      <button className="detail-submit-guest" type='button' onClick={() => handleBooking('pay_online')}>
      Thanh toán online
      </button>

      {!inputFilled ?
        <p className='text-center'>
          Nhập ngày và số lượng khách để kiểm tra tổng giá chuyến đi, bao gồm các khoản phí bổ sung và bất kỳ khoản thuế nào.
        </p>
        :
        <div>
          <p className='text-center'>
    ...
    <div>
      ...
      >
        <Box sx={{ ...styleGuest }}>
          ...
          <p className="guest-description">Nơi này có tối đa {khachMax} người</p>
          <button className="guest-close mb-4 ml-5" onClick={() => { setOpenGuest(false) }}>Đóng</button>
        </Box>
      </Modal>
        ...
        <Box sx={style}>
          <div className="detail-phone-box">
            <header className='booking-header'>
              ...
              <h3>{inputFilled ? "Xác nhận và thanh toán" : "Yêu cầu đặt phòng"}</h3>
            </header>
            <section className='booking-main' style={{}}>
              ...
              <section className='booking-room'>
                <h1 className='mt-2 mb-5'>Chuyến đi của bạn</h1>
                <div className='booking-room-date mb-3'>
                  ...
                  <h3 className='edit-booking' onClick={() => { setOpenDate(true) }}>Chỉnh sửa</h3>
                </div>
                <div className='booking-room-guest'>
                  ...
                  <h3 className='edit-booking' onClick={() => { setOpenGuest(true) }}>Chỉnh sửa</h3>
                </div>
              </section>
                ...
                <div>
                  <div className={`form-check ${!checkPayment ? "active" : ""}`}>
                    <label className="form-check-label" htmlFor="flexRadioDefault1">
                      ...
                      <p>{inputFilled ? `Trả đầy đủ ${giaTien * dateDifferent} USD` : "Chưa thể tính giá tiền"}</p>
                    </label>
                  ...
                  <div className={`form-check ${checkPayment ? "active" : ""}`}>
                    <label className="form-check-label" htmlFor="flexRadioDefault2">
                      <h2>Thanh toán một phần bây giờ, phần còn lại sau</h2>
                      <p>{inputFilled ? `Trả ${giaTien * dateDifferent} USD theo thời gian mà bạn chọn` : "Chưa thể tính giá tiền"}</p>
                    </label>
                    <div className='radio-button'>
              ...
              <section className='booking-paying-price'>
                ...
                <button className="detail-submit-guest" type='button' onClick={() => handleBooking('pay_at_property')}>
                  {!inputFilled ? "Kiểm tra tình trạng phòng" : "Thanh toán tại chỗ"}
                </button>
                <button className="detail-submit-guest" type='button' onClick={() => handleBooking('pay_online')}>
                  {!inputFilled ? "Kiểm tra tình trạng phòng" : "Thanh toán online"}
                </button>
              </section>
            </section>
            ...
            <footer className='booking-footer'>
              <div className='d-flex'>
                ...
                <h4>Tiếng Việt  $ USD</h4>
              </div>
              <p className='mt-4'>© 2023 Đăng Khoa & Minh Đức, Inc.</p>
              <p>Chính sách bảo mật · Điều khoản · Sơ đồ trang web</p>
            </footer>
          </div>
        </Box>
      </Modal>
