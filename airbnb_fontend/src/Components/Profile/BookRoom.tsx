...
function BookRoom({ bookRoom, index }: IProps) {
  ...
  useEffect(() => {
    try {
      const resp = async () =>
        await axiosInterceptorWithCybertoken.get(
          `/api/phong-thue/${bookRoom.maPhong}`,
        );
      resp().then((response) => {
        setBookRoom(response.data);
      });
    } catch (err) {
      console.log(err);
    }
        ...
        items={[
          {
            ...
            children: (
              <div>
                <h3>
                  Ngày đến:{" "}
                  <span style={{ fontWeight: "400" }}>{bookRoom.ngayDen}</span>
                </h3>
                <h3>
                  Ngày đi:{" "}
                  <span style={{ fontWeight: "400" }}>{bookRoom.ngayDi}</span>
                </h3>
                <h3>
                  Số khách:{" "}
                  <span style={{ fontWeight: "400" }}>
                    {bookRoom.soLuongKhach}
                  </span>
                </h3>
                <div className="d-flex justify-content-end">
                  <UpdateBooking
                    bookRoom={bookRoom}
                    khachMax={bookRoomList.khach}
                  />
                </div>
              </div>
            ),
